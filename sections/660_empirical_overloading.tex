...
\red{In particular, this lets us test the theory in both the \emph{Strongly Over-Parameterized Regime} $(\ND \gg N\times M)$ 
...
\subsubsection{Overloading FC1: \red{Strongly Over-Parameterized Regime} \texorpdfstring{$(\ND\gg N\times M$)}{n>>N x M}}
...
\chris{This might be a good place to cite Temperature Balancing, Layer-wise Weight Analysis, and Neural Network Training because this corroborates the idea that learning rates work differently on different layers.}
...
\subsubsection{Overloading FC2: \red{Under-Parameterized Regime} \texorpdfstring{$(\ND \ll N\times M$)}{n << NxM}}
