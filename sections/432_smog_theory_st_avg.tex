...
for both (and with $\ell_2$ loss),
 $\EPSL(\SVEC,\TVEC)$ is simply a function of the \emph{average} ST overlap $\AVGR$~\cite{SST92}.
This gives $\EPSLSTx\rightarrow\EPSL(\AVGR)$, where
\begin{align}
  \label{eqn:Rdef}
\AVGR=\frac{1}{\ND}\SVEC^{\top}\TVEC=\frac{1}{\ND}\sum_{i=1}^{m}s_{i}t_{i},
\end{align}
...
\red{Something is off.  This might be due to choosing the data vectors with variance $1/m$. Need to check this}
...
=& \ND - (\YsVEC)^{\top} (\YtVEC) \\
\label{eqn:deriveSTerror}
=& \ND(1 - \ETA(\SVEC,\TVEC,\XI)),
\end{align}
where we define $\ETA(\SVEC,\TVEC,\XI):=\tfrac{1}{\ND}\mathbf{y}_{S}^{\top}\mathbf{y}_{T}$, called
...
\ETA(\SVEC,\TVEC,\XI) = \tfrac{1}{\ND}(\SVEC^{\top}\XI)^{\top} (\TVEC^{\top}\XI) = \tfrac{1}{\ND}\XI^{\top} (\SVEC^{\top} \TVEC )\XI 
\end{align}
\red{Are we missing a $1/m$ somewhere}
or, more simply, after integrating over the data, we have the \emph{data-independent \SelfOverlap}
\begin{align}
  \label{eqn:eta_vec_avg_def}
\ETA(\SVEC,\TVEC) = \langle\ETA(\SVEC,\TVEC,\XI)\rangle_{\AVGNDXI}=\tfrac{1}{\ND}(\SVEC^{\top} \TVEC )
\end{align}
...
To do this, we need to compute the average or \ExpectedValue over all $\ND$ possible input data vectors $\XI$ (i.e., $d\mu(\NDXI)=\mathcal{D}\NDXI P(\NDXI)$).
%
...
\THRMAVG{1-\tfrac{1}{\ND}\SVEC^{\top}\TVEC}=
\THRMAVG{(1-\AVGR)}  ,
\end{align}
...
\Q^{ST} := 1 - \AVGSTGE(\AVGR) 
       ...
       = \THRMAVG{\tfrac{1}{\ND}\SVEC^{\top}\TVEC} 
       = \THRMAVG{\AVGR}.
\end{align}
