...
\end{align}
Since the data vectors are normalized to $1/m$, the overlap $\AVGR\in[-1,1]$.  And, importantly, the number of free parameters becomes $1$.
...
\red{THIS SECTION NEEDS TO BE REWORKED and has lots of errors/  SEE SECTION 5.1 for a much better presentation}

\begin{align}
\nonumber
\DETOPSTLL= & \frac{1}{2} \red{\operatorname{Tr}}(\YsVEC - \YtVEC)^{\top} (\YsVEC - \YtVEC) \\
\nonumber
...
In defining $\ETA(\SVEC,\TVEC,\NDXI)$, we replace the labels $(\mathbf{y}_{S},\mathbf{y}_{T})$ with the Energy functions $E_{NN}$  that generate them, giving an expression in terms of the weights $(\SVEC,\TVEC)$ and the Gaussian data variables $(\XI)$. We will then integrate out the data variables, leaving an expression just in terms of the weights.  
Using the $E_{NN}$ Energy generating or output function (\EQN~\ref{eqn:dnn_energy}, \ref{eqn:S_ENN}, \ref{eqn:T_ENN}), we can replace the label vectors $\YtVEC,\YsVEC$, where each of the $\ND$ elements if gi ven by
\begin{align}
\YsVEC_{\mu}=\SVEC^{\top}\XI_{\mu},\;\;
\YtVEC_{\mu}=\TVEC^{\top}\XI_{\mu}  ,
\end{align}
which gives
\begin{align}
  \label{eqn:eta_vec_xi_def}
\ETA(\SVEC,\TVEC,\NDXI) =\red{\sum_{\mu=1}^{\ND}} (\SVEC^{\top}\XI_{\mu})^{\top} (\TVEC^{\top}\XI_{\mu}) = \red{\sum_{\mu=1}^{\ND}}\XI^{\top}_{\red{\mu}}  (\SVEC^{\top} \TVEC )\XI_{\red{\mu}} 
\end{align}
...
\ETA(\SVEC,\TVEC) = \langle\ETA(\SVEC,\TVEC,\NDXI)\rangle_{\AVGNDXI}=\SVEC^{\top} \TVEC 
\end{align}
...
%
\red{BELOW IS WRONG NEEDS FIXED. SEE ALSO SECTION 5.1 below where the notation is more clear.  MAKE THESE CONSISTENT}
\begin{align}
