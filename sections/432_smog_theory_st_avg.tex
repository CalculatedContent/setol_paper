...
and in both the \LargeN \Thermodynamic limit in $\ND$, and "wide" \largeN  limit in $N$).)
...
\end{align}
Since the data vectors are normalized to $1/m$, the overlap $\AVGR\in[\red{-1},1]$.  And, importantly, the number of free parameters becomes $1$.
...
$\YsVEC:=[\YtIS{1}, \YtIS{2}, \cdots, \YtIS{\ND}]$.
This lets us write 
...
Using the $E_{NN}$ Energy generating or output function (\EQN~\ref{eqn:dnn_energy}, \ref{eqn:S_ENN}, \ref{eqn:T_ENN}), write
\begin{align}
\Ys=\SVEC^{\top}\XI_{\mu},\;\;
\Yt=\TVEC^{\top}\XI_{\mu}  .
\end{align}
...
\end{align}
After integrating over the data, we have the \emph{data-independent \SelfOverlap}
\begin{align}
...
= & \AVGR\;\frac{1}{\ND}\sum_{\mu=1}^{\ND}\int d\mu(\XI_{\mu})\XI_{\mu}^{\top}\XI_{\mu} \\ \nonumber
   = &\AVGR.
\end{align}
\nred{WHAT SHOULD WE SAY HERE ?
where the  holds because the elements of $\XI$ are i.i.d.
Since $d\mu(\NDXI)$ is a measure over a (multi-variate) Gaussian distribution (i.e., $d\mu(\NDXI)=\mathcal{D}\NDXI P(\NDXI)$), and because data vectors $\XI$ are normalized you unity (See Section~\ref{app:st-gen-err-annealed-ham}) .}

We can not obtain the \EffectivePotential $\EPSL(\AVGR)$ for the data-averaged ST test error, as in \EQN~\ref{eqn:epsl} as
\begin{align}
\label{eqn:e0}
