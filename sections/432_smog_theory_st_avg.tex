...
=& \ND - (\YsVEC)^{\top} (\YtVEC) \\
\label{eqn:deriveSTerror}
=& \ND(1 - \ETA(\SVEC,\TVEC,\red{\NDXI})),
\end{align}
where we define $\ETA(\SVEC,\TVEC,\red{\NDXI}):=
\tfrac{1}{\ND}\mathbf{y}_{S}^{\top}\mathbf{y}_{T}$, called
...
\ETA(\SVEC,\TVEC,\XI) =\red{\tfrac{1}{\ND}\sum_{\mu=1}^{\ND}} (\SVEC^{\top}\XI)^{\top} (\TVEC^{\top}\XI) = \red{\tfrac{1}{\ND}\sum_{\mu=1}^{\ND}}\XI^{\top}_{\red{\mu}}  (\SVEC^{\top} \TVEC )\XI_{\red{\mu}} 
\end{align}
\red{THIS SECTION HAS SOME TYPOS}
...
\THRMAVG{1-\cancel{\tfrac{1}{\ND}}\SVEC^{\top}\TVEC}=
\THRMAVG{(1-\AVGR)}  ,
\end{align}
...
\Q^{ST} := 1 - \AVGSTGE(\AVGR) 
       ...
       = \THRMAVG{\cance;{\tfrac{1}{\ND}}\SVEC^{\top}\TVEC} 
       = \THRMAVG{\AVGR}.
\end{align}
