...
\red{As described in Figure~\ref{fig:data_mapping},}
...
\red{and $\XImu$ is normalized such that $\Vert\XImu\Vert=\tfrac{1}{M}$ for all $N$ data vectors.}
...
%%5\AVGSTGE:= & \left\langle\THRMAVG{\Delta\mathbf{E}_{\mathcal{L}}(\SVEC,\TVEC, \XI)}\right\rangle_{\NDXI} 
%%5\end{align}
...
%%5\langle\Delta\mathbf{E}_{\mathcal{L}}(\SVEC,\TVEC,\XI)\rangle_{\NDXItrain} := \dfrac{1}{\Ntrain}\sum_{\mu=1}^{\Ntrain}\Delta\mathbf{E}_{\mathcal{L}}(\SVEC,\TVEC,\XI_{\mu})
%%5\end{align}
...
\EPSLSTx = \langle\DETOPSTL\rangle_{\NDXI}:=\frac{1}{N}\int d\mu(\NDXI)\DETOPSTL
\end{align}
...
\michaeladdressed{MM TO DO: maybe point to App A2 here.}
\michaeladdressed{@charles: I tweaked the wording in that par; double check that I got the logical flow correct.}
\michaeladdressed{One minor issue is that \EQN~\ref{eqn:finalEgen} has a subscript $w$ and here we have a subscript $s$.}
...
\end{align}
which is simply the dot product between the \red{$M$-dimensional} \Student $\SVEC$ and \Teacher $\TVEC$ weight vectors, \red{and normalized by the number of training instances $N$}.
...
\ETA(\SVEC,\TVEC) = \langle\ETA(\SVEC,\TVEC,\XI)\rangle_{\XI}=\tfrac{1}{N}(\SVEC^{\top} \TVEC )
\end{align}
...
\begin{align}
  \langle\DETOPSTLL\rangle_{\NDXI}  
   = &\frac{1}{\mathcal{N}}\int d\mu(\NDXI)(1-\ETA(\XI)) \\ \nonumber
   = &\frac{1}{\mathcal{N}}\int d\mu(\NDXI)(1-\XI^{\top}\SVEC^{\top}\TVEC\XI) \\ \label{eqn:XI_ST}
   = & \frac{N}{\mathcal{N}} - \frac{1}{\mathcal{N}}\int d\mu(\NDXI)\XI^{\top}\SVEC^{\top}\TVEC\XI \\ \nonumber
...
if we normalize the data vectors $\XI$ to $\frac{1}{\sqrt{N}}$,  then $\mathcal{N}=N$, and we recover 
\begin{align}
\label{eqn:e0}
\EPSL(R)=\langle\DETOPSTLL\rangle_{\NDXI} =  1 - R .
\end{align}
%%%
%%% CHM: what is this ?
%%\begin{align}
...
\THRMAVG{1-\langle\ETA(\SVEC,\TVEC,\XI)\rangle_{\XI}}=
...
=\THRMAVG{\langle\ETA(\SVEC, \TVEC, \XI) \rangle_{\XI}}
