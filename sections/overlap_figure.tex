...
\textbf{Left:} \(\AVGR = \mathbf{s}^\top \mathbf{t}\) (implicitly normalized to $1/m$).
\textbf{Right:} \(R = \tfrac{1}{N}\,\mathrm{Tr}\bigl(\mathbf{S}^\top\mathbf{T}\bigr)\) with conic sections on the sphere (red \(\mathbf{S}\), blue \(\mathbf{T}\)), plus a purple wedge for the angle.  Averaged over matrix dimensions $N$ and $M$ (implicitly normalized over to $1/M$).}
\label{fig:overlaps}
\end{figure}
