...
\textbf{Left:} \(\AVGR = \mathbf{s}^\top \mathbf{t}\) (and  normalized s.t. $\Vert\SVEC\Vert_{F}=\Vert\TVEC\Vert_{F}=1$).
...
Note that $R\in[0,1]$, where $R=1$ indicates Student perfect learning (and $R\ge0$ there no Students that predict more than half the Teacher labels incorrectly).
}
\label{fig:overlaps}
\end{figure}
