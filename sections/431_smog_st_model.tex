...
Let us say we have a model, called \Teacher $(T)$, which maps some \emph{actual} (i.e., correlated\red{,  meaning the features are not i.i.d.,}) data
...
the NN output \cyan{ or inference energy} function $\NNOUT$
\footnote{
...
whereas for training a \red{DNN} model, it is usually something like a cross-entropy loss.
%, but this detail does matter later.
\footnote{\cyan{Note that $\AVGGE^{T}$ will be treated as an energy function, as is $\NNOUT$, yet they are related nonlinearly. This is because they are energies in \emph{two different systems} -- training, and inference -- and $\mathcal{L}$ is a transfer function that bridges them.}}
...
In \STATMECH, even in the AA and at high-T, the average over Student weights $\langle\cdots\rangle_{S}$ is a \ThermalAverage $\THRMAVG{\cdots}$.  This means we should restrict the choice of $S$ to the same Boltzmann distribution we assume $T$ is 
drawn from.  In a practical sense, we would expect our Students $S$ to be trained with the same amount of regularization (similar learning rates, weight decay, etc.). Being at high-T, however, we expect to allow some ''thermal fluctuations'' , which means we can admit a reasonable variety of Students in our average (i.e. as long as they overlap enough with  the Teacher.  Or, as shown later,  a very similar ESD.).
