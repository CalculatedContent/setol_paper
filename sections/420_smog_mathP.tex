...
%
\item 
  \textbf{From the ST Perceptron to q Matrix Model.} 
...
\end{equation}
\nred{I don't think this normalization is correct, double check}
...
\begin{align}
\langle \ln Z_n \rangle_{\AVGNDXIn} \approx \ln \langle Z_n \rangle_{\AVGNDXIn}, \quad \text{as } \ND \gg 1\;\; \text{(grows arbitrarily large)}.
\end{align}
...
In Section~\ref{sxn:matgen}, wee will move from a single $m$-dimensional \Perceptron vector $\WVEC$ 
...
whereas for the matrix case, we will retain the 'a reduced set' of the  $M$ degrees of freedom, $\mathcal{M}$.  To do this, we need to ensure the \FreeEnergies and related quantities like the \LayerQualitySquared to scale properly.
...
\end{itemize}
Note that by 'limit" we mean $\ND$ or $N$ simply grows arbitrarily large,  In taking both, we assume implicitly that the \emph{layer load}, $N/\ND$, the number of feature vectors per data point, remains constant, which is consistent with classical \STATMECH theories of learning\cite{MM17_TR} (although we do not enforce this explicitly here).  
...
and it ensure that our layer Free Energy, etc.,  remain well-behaved as we increase $M$.
...
Both \SizeExtensivity (in $\ND$ and $N$) and \SizeConsistency (in $\mathcal{M}$ )
...
\end{align}
that is evaluated in the ``\LargeN limit in $N$`` (here, meaning as $N \gg 1$, but not at the limit $N=\infty$).
