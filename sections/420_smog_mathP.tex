...
We need to compute various sums and integrals, sampling from a model $\MDD$ for the actual data distribution $\ADD$, and also over $\ND$-sized data samples (or data sets).
...
Thus, the implicit normalization $\tfrac{1}{\ND}$ ensures the \BraKet is a proper \ExpectedValue of a sample average.
%The measure $d\mu(\NDXIn)$ signifies a single $\ND$ vector $\XI$, drawn from an $m$-dimensional idealized Gaussian distribution.
A subscript $\NDXIn$ on the Ket as $\langle\cdots\rangle_{\XI}$ would represent an integral over potential data points, not an average of a data sample (i.e., there would be no $1/\ND$ prefactor).
...
A typical model in \STATMECH is to construct a Mean-Field Theory, formulated using a \SaddlePointApproximation (SPA). The SPA is a large-$\ND$ approximation and is used to describe the system interactions at large-$\ND$, forming the average or mean interaction over the data.
Here, we would write
