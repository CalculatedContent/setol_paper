...
\begin{align}
  \label{eqn:Emap}
  \DETOPX \xrightarrow{\text{expected value}} \langle \DETOPXI \rangle_{\NDXIn} %= \DETOT = n \EPSLw.
\end{align}
...
Finally, the normalization $\tfrac{1}{n}$ is included in the defintion to ensure the \BraKet is a proper \ExpectedValue.
...
\begin{align}
  \lim_{n\gg 1}  \langle\DETOPXI\rangle_{\NDXIn}=
...
The data-averaged error  $\langle \DETOPXI \rangle_{\NDXIn}$ will appear frequently below.
For compatibility with \cite{SST92}, we denote it using the symbol $\EPSLw$:
\begin{align}
 \label{eqn:epsl}
 \EPSLw:=\langle \DETOPXI \rangle_{\NDXIn} \quad \text{(\SizeIntensive)}.
\end{align}
...
\begin{align}
  \label{eqn:LCT}
  -\beta \bar{F}=\langle \ln Z \rangle_{\NDXIn} &= \sum_{\mu=1}^{\mathcal{M}}\text{(Connected Components)} \\ \nonumber
...
The full expected value  of the \FreeEnergy, $\beta F_{n}=-\ln Z_{n}$, with respect to the (model)
  data $\NDXIn$, as:
\begin{align}
  \mathbb{E}_{\NDXIn}[\beta F_n]=\beta\bar{F}:=-\langle \ln Z_{n}\rangle_{\NDXIn},
\end{align}
where $\mathbb{E}_{\NDXIn}[\beta F_n]$ is the \ExpectedValue of the \FreeEnergy, which we wish to model, and 
where $\langle \ln Z_{n}\rangle_{\NDXIn}$ means where we average over  the $n$ samples of the data .
...
.\nred{Note: when we use BraKets for Expected Values we mean in the physics sense and we have to specify the normalization explicitly }
...
\begin{align}
\label{eqn:AA}
-\langle\ln Z_{n}\rangle_{\NDXIn}\approx-\ln \langle Z_{n}\rangle_{\NDXIn}.
\end{align}
...
\end{align}
\nred{Actually, I think we want:
\begin{align}
   \beta\GAN:= - ln  \int d\mu(\NDXIn)e^{-\beta\DETOPXI}.
\end{align}
so that the $\tfrac{1}{n}$ is part of the definition of $d\mu(\NDXIn)$.
This gives
\begin{align}
  \label{eqn:Zan_simple}
  \ZAN &=  \int d\mu(\WVEC) exp\left[-n \left(- \ln  \int d\mu(\NDXIn)e^{-\beta\DETOPXI}\right)\right] \\ \nonumber
  &=  \int d\mu(\WVEC) exp\left[\ln \left( \int d\mu(\NDXIn)e^{-\beta\DETOPXI}\right)^{n}\right] \\ \nonumber
  &=  \int d\mu(\WVEC) \left( \int d\mu(\NDXIn)e^{-\beta\DETOPXI} \right)^{n} \\ \nonumber
  &=  \left (\int d\mu(\WVEC) \int d\mu(\NDXIn)e^{-\beta\DETOPXI} \right)^{n} \\ \nonumber
  &= [Z^{an}_{1}]^{n}
\end{align}
This would simply things greatly here and elsewhere 
}
...
%\end{align}
The order of the integrals is exactly what we expect using the AA, as in \EQN~\ref{eqn:switch}
