...
In this subsection, we will compare and contrast several types of averages, energies and other \Thermodynamic concepts we will encounter. 
...
%the goal is to explain is that one typically trains\red{only(?)} one large (expensive) NN, i.e., one does not necessarily need to split the data into training and testing sets (as in more traditional hyperparameter tuning of ML models\red{, because \SETOL makes \emph{data independent} predictions}.
%Thus,) 
...
%\red{In this training process, labels are provided, and are used in producing gradients and so on, but in our theory we do non consider them as a form of supervision. Instead, we}
...
%\footnote{To be more precise, $M$ is defined to be the lesser dimension of $\WMAT$ and $N$ the greater. For simplicity we assume that $M$ is always the input dimension and $N$ the output dimension. Under the assumptions of \SETOL, there is no loss of generality from doing this.} The number of data samples will remain $\ND$ throughout.
...
Here, we treat %$\MY_{\mu}$ as a deterministic output of the Teacher model $T$, 
the labels $\MY$ as
% meaning that $\MY_{\mu}$ is 
...
\charles{Finally, in this preliminary Section, we represent the dimensions with lower case $n,m$, but elsewhere
  (and sometimes below), we will use capital $N,M$.}
...
\beta F_{\ND}:=-\ln Z_{\ND}.
\end{align}
Keep in mind that $Z_{\ND}$ may actually be a function of the \red{idealized} data $\XI$ (or some other variables),
...
\end{align}
These equations guarantee that the solution $\EPSL(\WVEC^{*})$ is at \red{a local} minimum in $\WVEC$.
...
For NNs, \SizeConsistency appears when scaling the number of \red{feature vectors} $N$ in our matrix model,
