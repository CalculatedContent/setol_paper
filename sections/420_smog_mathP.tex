...
\emph{Large-$\ND$ Approxmation} and the \emph{SaddlePointApproximation} (SPA),
...
\item 
  ...
  In Section~\ref{sxn:largeN_and_SPA}, we discuss the large‐$\ND$ \ThermodynamicLimit and the \SaddlePointApproximation (SPA).
...
We call $\XVEC^{\ND}$ an $\ND$-dimensional sample (of the training data instances $\XVEC$) from $\ADD$. \red{I'm really not comfortable calling $\ND$ the only dimension. $\XVEC_{\mu}$ is $\ND \times m$ dimensional.} 
...
\begin{align}
  \label{eqn:DEy}
  \Delta E^{\red{2}}_{\mathcal{L}}(\WVEC,\XVEC_{\mu},\MY_{\mu}):= (\MY_{\mu}-E_{NN}(\WVEC,\XVEC_{\mu}))^{2}  ,
\end{align}
\red{Are we sure it's accurate to call a squared difference ``$\DEL^{2}$''? A $\Delta$ of something should just be a difference.}
where 
%MM%the loss is the L2 loss, $\mathcal{L}=\ell_2$, and 
$E_{NN}(\WVEC,\XVEC_{\mu})$ is output prediction of the (student) NN, as in \EQN~\ref{eqn:dnn_energy}.
...
\end{align}
where the BraKet $\langle\cdots\rangle_{\AVGNDXIn}$ denotes the integral over the idealized
Gaussian-field data $\NDXIn$, but with the convention that the normalization $\tfrac{1}{\ND}$
...
\end{align}
where $P(\NDXIn)$ is an $\ND$-dimensional Gaussian distribution,
%probability distribution (i.e., an $\ND$-dimensional Gaussian distribution), 
normalized to 1, and where the subscript $\AVGNDXIn$ on the Ket indicates this is an %average or 
\ExpectedValue of a finite, $\ND$-size \emph{sample} of the ideal data -- note how the probability measure becomes a product of measures.
(This is used in both Sections~\ref{sxn:SMOG_main} and~\ref{sxn:matgen}.)
The normalization $\tfrac{1}{\ND}$ is included in the definition to ensure the \BraKet is a proper \ExpectedValue of an average.
...
\red{This is not quite right -- a BraKet value is always an Expected Value of something, by definition. Here we're saying we want it to be the Expected Value \emph{of} a sample average. The expectation is taken over samples, and the average is computed from each sample.}
...
\michaelB{Q: Is that a typo? Now, I'm confused, isn't there an extra $\frac{1}{\ND}$ factor there. ()}
...
where $\ND$ is the number of free parameters ($n=N$ for a vector model, but a matrix model has $N\times M$ free parameters; see Appendix~\ref{app:st-gen-err-annealed-ham}).
...
\begin{eqnarray*}
    \beta\GAN:=  -\tfrac{1}{\ND}\ln\langle e^{-\beta\DETOPXI}\rangle_{\NDXIn}=  -\ln \langle e^{-\beta\DET\rangle_{\XI}.
\end{eqnarray*}
...
\subsubsection{The Thermodynamic limit and the Large-n limit.}
\label{sxn:largeN_and_SPA}
The \ThermodynamicLimit is the large-$\ND$ limit of the model, as $\ND$ grows arbitrarily large, i.e. $n \gg 1$.
...
\paragraph{The large-N limit and the \SaddlePointApproximation (SPA).}
To evaluate $\QT$ we will be in both 
 the \ThermodynamicLimit and a different large-N limit
...
Later, when we form our matrix generalization of the ST model, we will also take a large-$\ND$ limit, and in doing so, the effective load $N/\ND$ will effectively remain constant (although this is not strictly enforced) and, in doing so, require that the final result will be \SizeConsistent.
We can approximate the asymptotic behavior of integrals we will encounter (e.g., the \FreeEnergy $F$ and/or \PartitionFunction $Z$) in a large-N limit by using the \SaddlePointApproximation (SPA), where $\ND$ a matrix dimension now. In the case of the $F$ and/or $Z$, we assume that we can apply the SPA:
\nred{Replace $\EPSL$ with $\GANMAT$ or NM\EPLS
\begin{align}
  \label{eqn:SPA}
  \int d\mu(\WVEC) e^{-\ND\red{\ND}\beta\EPSL(\WVEC)}\approx \sqrt{\tfrac{2\pi}{\ND\EPSL(\WVEC^{*})}} e^{-n\beta\EPSL(\WVEC^{*})}  ,
\end{align}
