...
Notice that $\XVEC_{\mu}\in\mathbb{R}^{m}$ is normalized such that the Frobenius norm is $\tfrac{1}{m}$:
  \begin{equation}
    \label{eqn:XVEC_norm}
...
\end{align}
and if we want to include the normalization then we would express this as 
\begin{align}
    \label{eqn:EuT_normalized}
    \langle f(\XI) \rangle_{\AVGNDXIn} := \frac{1}{n} \int d\mu(\XI) f(\XI)  .
\end{align}
...
\michalB{Q: Is that eqn a typo, should be \EQN~\ref{eqn:average_data_sample_error}? If yes, this sentence should be up there.}
We also call $\EPSLw$ the \emph{\EffectivePotential}, which will be made clear below.
\michaelB{Some of the last ca 12 lines is redundant, but before I change anything, I want to clarify those thigns.}
...
\begin{equation}
    \Gamma = 1-F .
\end{equation}
\michaelB{Q: do I want one-minus, or just minus? See question above about error being less than $1$.}
This quantity decreases as the error increases (as opposed to $F$, which increases), i.e., it increases as the accuracy of quality of the model increases.
...
\end{align}
\michaelB{Q: Is this the AA backwards? I.e., not the AA but a swap that we can do due to it?}
...
Following~\cite{SST92} (see their \EQN(2.30)), we will call this the \emph{\Annealed Hamiltonian} $\GAN$, 
...
\end{align}
Note also that the order of the integrals is exactly what we expect using the AA, as in \EQN~\ref{eqn:switch}.
