...
by forming an
expected value over the model data, $\langle\cdots\rangle_{\AVGNDXIn}$,
using the notation:
\begin{align}
  \label{eqn:Emap}
  \DETOPX \xrightarrow{\text{expected value}} \langle \DETOPXI \rangle_{\AVGNDXIn} %= \DETOT = n \EPSLw.
\end{align}
...
\begin{align}
   \beta\GAN:= - ln \tfrac{1}{n} \int d\mu(\NDXIn)e^{-\beta\DETOPXI}.
\end{align}
so that the $\tfrac{1}{n}$ is inside the logarithm.  
This gives
\begin{align}
  \label{eqn:Zan_simple}
  \ZAN &=  \int d\mu(\WVEC) exp\left[-n \left(- \ln \tfrac{1}{n} \int d\mu(\NDXIn)e^{-\beta\DETOPXI}\right)\right] \\ \nonumber
  &=  \int d\mu(\WVEC) exp\left[\ln \left(\tfrac{1}{n} \int d\mu(\NDXIn)e^{-\beta\DETOPXI}\right)^{n}\right] \\ \nonumber
  &=  \int d\mu(\WVEC) \left( \tfrac{1}{n}\int d\mu(\NDXIn)e^{-\beta\DETOPXI} \right)^{n} \\ \nonumber
  &=  \left (\int d\mu(\WVEC) \tfrac{1}{n}\int d\mu(\NDXIn)e^{-\beta\DETOPXI} \right)^{n} \\ \nonumber
  &= [Z^{an}_{1}]^{n}
\end{align}
This further illustrates how the AA results in a kindof mean-field approximation.
}

%or, in Bra-Ket notation
%\begin{align}
%%  \label{eqn:Zan_brakets}
%  \ZAN = \langle\langle e^{-\beta\DETOPXI}\rangle_{\AVGNDXIn}\rangle_{\WVEC}
%\end{align}
