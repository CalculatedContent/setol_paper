...
%\noindent
To answer these questions, we will derive a general expression for the \LayerQuality, $\Q$, of an NN \red{in terms of the spectrum (eigenvalues) of a layer weight matrix, which is in turn expressed in terms of the R-transform}.
...
In deriving these quantities, we will introduce to NN theory a new \SemiEmpirical approach that combines techniques from \STATMECH and \RMT in a novel way. \red{It is \SemiEmpirical because we derive from first principles the fact that model quality must depend on the spectral density of the weight matrix, but in practice, we must empirically determine the density's parametric functional form in order to derive an expression of model quality.}
...
In deriving $\Q$, we have discovered a new \LayerQuality metric, \red{induced by a necessary condition} called the \TRACELOG condition. The \TRACELOG condition indicates that the generalizing components of the layer \red{must} concentrate into a low-rank subspace\red{, which we term} the \emph{\EffectiveCorrelationSpace}, or~\ECS. \red{That is, the better this condition is met, the more the generalizing components concentrate in this \ECS, and this tendency provides a new layer quality metric, derived in a totally independent way from \ALPHA and \ALPHAHAT. We will provide strong empirical justification that both metrics are acceptable Quality metrics by showing that they converge as model quality increases.}
