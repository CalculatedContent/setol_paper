...
\end{equation}
\red{The overlap matrix $\OVERLAP$ describes the average interactions between $N$ interacting $M$-dimensional layer (i.e., feature) vectors.  We have dropped $1/\ND$ term because the matrix $\TMAT$ is an empirical ''measurable'' and therefore  $\TMNAT$ and $\SMAT$ are implicitly averaged over the data.
}
...
\end{align}
\nred{We normalize the $\IZG$ because we are going to take the \LargeN limit in $N$, and we need to keep the \Quality finite.  This allows us to implicitly fix the effective load of the layer, $N/\ND$, as $\IZG$ is also in the \ThermodynamicLimit and therefore at \LargeN in $\ND$, although we do not explicitly set $N/\ND$.  This is similar in spirit to an infinitely wide network.}
