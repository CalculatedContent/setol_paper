...
\end{align}
\nred{We normalize the $\IZG$ because we are going to take the \LargeN limit in $N$, and we need to keep the \Quality finite.  This allows us to implicitly fix the effective load of the layer, $N/\ND$, as $\IZG$ is also in the \ThermodynamicLimit and therefore at \LargeN in $\ND$, although we do not explicitly set $N/\ND$.}
