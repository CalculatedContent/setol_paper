...
\begin{align}
  ...
  =\frac{1}{N^{2}}\Trace{\TMAT^{\top}\mathbf{A}_N\TMAT}  .
\end{align}
This choice places $\QT$ in the form of the HCIZ integral, as in \EQN~\ref{eqn:hciz_prelim}.
See Appendix~\ref{sxn:tanaka} for a detailed discussion of why we choose $\AMAT=\AMAT_N$ here.
...
We have two choices for the \Student \CorrelationMatrix $\mathbf{A}$, call them $\mathbf{A}_M$
and $\mathbf{A}_N$, defined as:
\begin{eqnarray}
  \label{eqn:A1}
  \mathbf{A}_M &:=& \frac{1}{N}\mathbf{S}^{\top}\mathbf{S} \quad\mbox{(which is $M \times M$)} \\
  \label{eqn:A2}
  \mathbf{A}_N &:=& \frac{1}{N}\mathbf{S}\mathbf{S}^{\top} \quad\mbox{(which is $N \times N$)} .
\end{eqnarray}
...
We note that $\mathbf{A}_M$ and $\mathbf{A}_N$ have the same eigenvalues $\lambda_{i}$, or ESD,
up to the additional zero eigenvalues $(\lambda_{i}=0)$ in the null space of $\mathbf{A}_N$.  
Consequently, both forms of $\AMAT$ have the same $N-M$ non-zero eigenvalues, the same non-zero part
of the ESD $(\rho(\lambda)\gg 0)$, and the same Trace $(\Trace{\mathbf{A}_M}=\Trace{\mathbf{A}_N})$.
...
$\mathbf{A}_M$  with $\mathbf{A}_N$, such that
\begin{align}
 \int d\mu(\mathbf{A})\;[\cdots]\;\leftrightarrow  \int d\lambda\;[\cdots]\; \rho^{\infty}_{\mathbf{A}_M}(\lambda) \leftrightarrow  \int d\lambda\;[\cdots]\; \rho^{\infty}_{\mathbf{A}_N}(\lambda)
\end{align}
...
$\AECS$ instead of explicitly using  $\mathbf{A}_M$ and $\mathbf{A}_N$.
...
in terms of $\mathbf{A}_N$, giving, as in Eqn.~\ref{eqn:QT_dS}, 
\begin{align}
  \label{eqn:IZG_dmuS}
  \IZG = \ln \int d\mu(\SMAT)  e^{ N\beta Tr[\tfrac{1}{N}\TMAT^T\mathbf{A}_N\TMAT]) }  .
\end{align}
To recast \EQN~\ref{eqn:IZG_dmuS} as an HCIZ integral, as in \EQN~\ref{eqn:hciz_prelim},
we must perform change of measure,
 from $N\times M$ \Student weight matrices $\mathbf{S}$ to $N\times N$ \Student Correlation matrices $\mathbf{A}_N$, as in \EQN~\ref{eqn:changeOfMeasure}.
...
\begin{align}
  ...
  &\approx 
  \ln \int d\mu(\AMAT)
  e^{N\beta Tr[\tfrac{1}{N}\TMAT^{\top}\AMAT_N\TMAT] }
  e^{\tfrac{N}{2}\ln(\Det{\AMAT_M})} \\
  & = 
  \ln
  \left\langle
  e^{N\beta Tr[\tfrac{1}{N}\TMAT^{\top}\AMAT_N\TMAT] }
...
it contains the original overlap term that depends $\mathbf{A}_N$ as well as a new term from
the transformation that depends on $\Det{\AMAT_M}$ that is not yet defined.
...
student layer correlation matrix, $\mathbf{A}_M=\tfrac{1}{N}\mathbf{S}^{\top}\mathbf{S}$
(or, equivalently, $\mathbf{A}_N$).
...
(in either form, $\mathbf{A}_M$ or $\mathbf{A}_N$).
\begin{align}
...
$|\lambda_{i}\rangle$  associated with the eigenvalue $\lambda_{i}$ of $\mathbf{A}_M$ or, equivalently, $\mathbf{A}_N$.
We denote the corresponding \Student correlation matrices with tilde, as follows:
\begin{align}
  \AMAT_M \rightarrow \AECSM\;\; \text{such that}\;\; d\mu(\AMAT_M) \rightarrow d\mu(\AECSM) \\ \nonumber
  \AMAT_N \rightarrow \AECSN\;\; \text{such that}\;\; d\mu(\AMAT_N) \rightarrow d\mu(\AECSN)  \nonumber
\end{align}
...
and
$\rho_{ECS}(\lambda)\supseteq\rho_{tail}(\lambda)$, as depicted in Figure \red{XXX}.
...
\begin{quote}
  ...
  and, importantly, such that $\det(\AMAT)=\det(\AMAT_M)=\det(\AMAT_N)$ is well defined.  
\end{quote}
...
\begin{align}
  ...
  &=
  \ln \int d\mu(\AECS)
  e^{N\beta Tr[\tfrac{1}{N}\TMAT^{\top}\AECSN\TMAT] }
...
$\langle \exp\left[\tfrac{N}{2} \Trace{ \ln[det\;\AMAT_M] } \right]\rangle_{\AECS}=1$,
...
(either $\AECSM$ or $\AECSN$)  (in the~\ECS)  is the point estimate provided by the actual (known) fixed \Teacher correlation matrix $\XECS$, so that
\begin{align}
\label{eqn:IFA-ppint}
...
$\langle\Det{\mathbf{A}_M}\rangle$ itself of the determinant
of a random full rank \Student Correlation matrix $\mathbf{A}_M$
...
$\langle\Det{\mathbf{A}_M}\rangle$  with an empirical point estimate over the \Teacher Correlation matrix, which is unity.
\begin{equation}
\label{eqn:detX}
...
We also refer to \EQN~\ref{eqn:detX} the \TRACELOG condition, which is technically its empirical form.
