...
\end{align}
where $\AMATN$ is the $N\times N$ form of the \Student correlation matrix,
$\mathbf{A}_N:=\frac{1}{N}\mathbf{SS}^{\TR}$.
We will also define the $M\times M$ matrix, $\AMATM=\frac{1}{N}\mathbf{S}^{\TR}\mathbf{S}$,
used later. Note that $\AMATM$ also has a $\frac 1 N$ factor, so that its non-zero eigenvalues will be the same as $\AMATN$, but has dimensions $M \times M$.
...
\begin{align}
  \label{eqn:QT_dS}
  \IZG  &=  \ln\int d\mu(\mathbf{S})\exp\left(N\beta\Trace{\mathbf{T}^{\TR}\mathbf{A}_N\mathbf{T}}\right),
\end{align}
...
%  Importantly, this lets interpret the \EQN~\ref{eqn:QT_HCIZ}  as acting only in this subspace.
  ...
  The Effective \Student correlation matrix $\AECSM$ 
  satisfies the condition that $\Trace{\ln\AECSM}=\ln\Det{\AECSM}=0$,
  so that the change of measure $d\mu(\SMAT) \rightarrow d\mu(\AECS) $ is Volume Preserving.
  This condition is derived explicitly in terms of $\AECSM$, and therefore will hold for $\AECSN$
  (and by assumption the Teacher Correlation matrix, $\XECS=\frac{1}{N}\TMAT^{\TR}\TMAT=\frac{1}{N}\mathbf{P}_{ecs}\TMAT^{\TR}\TMAT$).
...
\begin{itemize}
  ...
  \IZGINF = \lim_{N\gg 1}\ln\int d\mu(\AECS)\exp\left(\beta\Trace{\mathbf{T}^{\TR}\AECSN\mathbf{T}}\right) %= \beta\sum_{i=1}^{\MECS}\GNECS
  \end{align}
  where  measure $d\mu(\AECS)$ lets us average over all \Student Correlation matrices $\AECSN$ which
