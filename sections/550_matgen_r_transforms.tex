...
\begin{figure}[h]
    ...
    \subfigure[IMP Distribution, $\kappa=0.5$]{ 
      ...
      \caption{Example Inverse Marchenko-Pastur (IMP) distributions for $\kappa=0.5$ and $\kappa=1.0$,  along with Power Law (PL) fits of the generated distribution. Plots on Log-Log scale.}
  \label{fig:IMPplots}                                                                                                      
\end{figure}   
...
\nred{Actually, I think we only need the real part, I need to redo this part}
...
\nred{Probably we just want the real part, I think with the Tananka integral the imaginary parts cancel; need to check again}
\begin{equation}
