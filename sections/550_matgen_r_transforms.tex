...
here, we consider the \InverseMP (IMP) model.
...
\nred{I need to think this section through a bit more.  I think we may need more than 1 adjustable parameter, and the branch cut may or may not matter depending on those choices}
%
...
\begin{figure}[h]
    \centering
    \subfigure[IMP Distribution, $\kappa=0.25$]{ 
      \includegraphics[width=7cm]{./img/IWplotQ1.5.png}
      \label{fig:IMPplotQ15}                               
    }                               
      \subfigure[IMP Distribution, $\kappa=1.25$]{ 
      \includegraphics[width=7cm]{./img/IWplotQ3.5.png}
      \label{fig:IMPplotQ35}                                
      }
      \caption{Example Inverse Marchenko-Pastur (IMP) distributions for $\kappa=0.25$ and $\kappa=1.25$,  along with Power Law (PL) fits of the generated distribution. Plots on Log-Log scale.\red{THESE NEED TO BE REDONE}}
  \label{fig:IMPplots}                                                                                                      
\end{figure}   
...
Importantly, as seen in Figure~\ref{fig:IMPplotQ15}, for $\alpha= 2.0$, the IMP model (with $\kappa=0.25$)
is an effective, simple model to illustrate the \SETOL case of \IdealLearning.

\begin{figure}[t]
    \centering
    \subfigure[IMP $R(z)$, real $z$.]{
        \includegraphics[width=7cm]{img/IW_R_transform_sides.png}
        \label{fig:IMP_R}
    }
    \subfigure[Branch cut at $z=\kappa/2$]{
        \includegraphics[width=7cm]{img/Branch_Cut_Density_normalized.png}
        \label{fig:IMP_branch_cut}
    }
    \caption{(a) The function $R(z)$ of the Inverse MP model, with a singularity at $z = \kappa/2$. (b) The branch cut in the empirical spectral density, corresponding to the tail for $\kappa = 0.5$.}
...
\GN[\mathrm{IMP}] := \int_{\LambdaECSmin}^{\lambda} R(z)[\mathrm{IMP}] dz  ,
\end{equation}
...
To correct for this, we will instead model the \LayerQualitySquared using the modulus of $\GN[\mathrm{IMP}]$,
\begin{equation}
\label{eqn:IMP_model_2} 
|\GN[\mathrm{IMP}]| := \sqrt{\GN[\mathrm{IMP}]^{*}\GN[\mathrm{IMP}]}
\end{equation}
where $\GN[\mathrm{IMP}]^{*}$ is the complex conjugate of $\GN[\mathrm{IMP}]$.
This is somewhat involved, so we present the full calculation in Appendix~\ref{sxn:IMP}
\michael{MM TO DO: Go through.}
\michael{Are we going to mention the result here since we will use it?}


Figure~\ref{fig:InverseMPGx} plots $|\GN[\mathrm{IMP}]|$ on Lin-lin and Log-log plots, on the range $\lambda\in(0.25, 100)$, and fits the it to a PL. 
...
\begin{figure}[t]
  \centering
  \subfigure[$|\GNECSI\lbrack IMP\rbrack|$ Lin-lin plot and PL fit]{
    \includegraphics[width=0.45\textwidth]{./img/Gx.png}
        \label{fig:GxPlot}
    }
    \subfigure[$|\GNECSI\lbrack IMP\rbrack|$ Log-log plot and PL fit]{
        \includegraphics[width=0.45\textwidth]{./img/logGx.png}
        \label{fig:LogLogGxPlot}
    }
    \caption{Behavior of $\GN[\mathrm{IMP}]$ for the Inverse MP (IMP) model,
       with a Power Law (PL) fit (red), $|\GN[\mathrm{IMP}]|\approx 1.138 \lambda^{0.539}$.
      (a)  Lin-lin plot. (b) Log-log plot.
}
    \label{fig:InverseMPGx}
\end{figure}
...
%        \label{fig:LogLogGxPlot}
%    }
%    \caption{Behavior of $\GNECSI$ for the Inverse MP (IMP) model. (a) $\GNECSI$ lin-lin plot. (b) Log-log plot of $\GNECSI$ with a Power Law (PL) fit, $\GNECSI\sim 2.8\lambda^{-3.0}$ }
%    \label{fig:InverseMPGx}
%\end{figure}
%
%Given an analytic expression for $\GNECSI$ (and $\kappa=0.25$), we can plot the $\GNECSI$ as a function of $\lambda$,
%and then fit this to a Power Law; this is depicted in Figure~\ref{fig:InverseMPGx}.
