...
\red{This is new, needs cleaning up}%--------------------------------------------------------------------
\paragraph{Step 2.0} We start with the \emph{expected} partition function
\begin{align}
  \label{eqn:avg_ZD}
  \EZDA
  &=
  \int d\mu(\AMAT)\int d\WMAT
      ...
      \Bigr].
\end{align}
The average over \AMAT\ affects only the first exponential; we
...
\paragraph{Step 2.3}
We now insert \ref{eqn:dmuX} into~\ref{eqn:avg_ZD},  which lets
...
\nred{Check for typos above, maybe missing a $1/2$}
\paragraph{Step 2.4}
