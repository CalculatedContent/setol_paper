...
\begin{equation}
  \label{eqn:XInorm}
  \Vert \NDXI \Vert^2 :=\cancel{\frac{1}{\ND}} \sum_{\mu=1}^{\ND} \XI_\mu \XI_\mu^{\top} = \sigma^{2}\IM ,
\end{equation}
...
The definition of the \AnnealedHamiltonian, $\GANMAT$, for the idealized case must be extended to account for the $N\times M$ parameters per training example.  We then have that the total energy is then the sum of the entries of $M$ independent parts, as expected by \SizeConsistency:
\begin{align}
 \label{eqn:hanpp}
...
\IH &=   \NORM\left[\frac{(2\pi\sigma^{2})^{M/2}}{\sqrt{\Det{ \mathbf{M}}}}\right]^{\ND} \\ \nonumber
    &=   \NORM(2\pi\sigma^2)^{NM/2}\left[
         \sqrt{\Det{ 2\beta\sigma^2\red{N}(\IM-\tfrac{1}{}\SMAT^{\top}\TMAT)+\IM}}\right]^{-\ND}  \\
%%\end{align}
%%%
%%\begin{align}
\label{eqn:I_6}
