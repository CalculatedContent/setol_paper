...
This now lets us write the Average Model \TrainingError $\AVGSTTE$ in the AA in terms of the \AnnealedHamiltonian $\GANR$:
  ...
  \end{align}
  where $\langle \mathbf{E}^{\beta}_{\ell_2}(\SVEC,\TVEC,\XI) \rangle_{\XItrain}$
  is a \ThermalAverage but defined over the specific ST error in the AA for the chosen set of training data $\XItrain$, and is given by
  \nred{CLEAN THIS UP}
 \begin{align}
   \langle  \mathbf{E}^{\beta}_{\ell_2}(\SVEC,\TVEC,\XI) \rangle_{\XItrain}:==\dfrac{\partial \beta\HAN}{\partial \beta}.
 \end{align}
