...
\begin{align}
\label{eqn:Gan0}
\beta\GANR = \beta\HAN(\beta,\SVEC,\TVEC):=-\cancel{\red{\frac{1}{N}?}}\ln\red{\frac{1}{N}}\int d\mu(\NDXI)e^{-\beta\DETOPSTLL}  .
\end{align}
...
This now lets us write the Average Model \TrainingError $\AVGSTTE$ in the AA in terms of the \AnnealedHamiltonian $\GANR$:
 ...
 \nred{not sure if we want to move the $\tfrac{1}{N}$ inside the log or not yet}
 \begin{align}
  \label{eqn:partial_Gan}
  \dfrac{\partial \beta\HAN}{\partial \beta}
  &=  \dfrac{\partial }{\partial \beta} \left(-\cancel{\red{\frac{1}{\ND}}?}\ln \red{\frac{1}{\ND}} \int d\mu(\NDXIn)e^{-\beta\DETOPXI}\right) \\ \nonumber
  &=  -\cancel{\red{\frac{1}{\ND}}}\dfrac{\partial }{\partial \beta} \ln \red{\frac{1}{\ND}}\int d\mu(\NDXIn)e^{-\beta\DETOPXI} \\ \nonumber
  &=  -\cancel{\red{\frac{1}{\ND}}} \left( \int d\mu(\NDXIn)e^{-\beta\DETOPXI}\right)^{-1}\dfrac{\partial }{\partial \beta} \red{\frac{1}{N}}\int d\mu(\NDXIn)e^{-\beta\DETOPXI} \\ \nonumber
    &=  -\cancel{\red{\frac{1}{\ND}}} \left(\int d\mu(\NDXIn)e^{-\beta\DETOPXI}\right)^{-1} \red{\frac{1}{N}}\int d\mu(\NDXIn)(-\DETOPXI)e^{-\beta\DETOPXI}
...
\nred{We might (need to) change this to
  $\beta\HANPP:=-\ln\frac{1}{N}  \int\mathcal{D}\NDXI \, e^{-\beta \DETOPST} P(\NDXI) $
  but this will require rederiving the final result and ensuring it is stil correct.
}
