...
%%   \ln \langle Z \rangle_{\NDXI} = 
   \ln \int d\mu(\NDXI) \int d\mu(\SVEC) e^{-\beta\DETOPSTLL}   .
\end{align}
%
...
\langle-\beta F \rangle_{\NDXI} \simeq 
%%   \ln \langle Z \rangle_{\NDXI} = 
   \ln \int d\mu(\SVEC)\int d\mu(\NDXI)e^{-\beta\DETOPSTLL)}   .
\end{align}
...
\end{align}
\nred{CLEAN UP THE ABOVE. L2 is wrong. Is there an extra $\ND$.  $n\rightarrow\ND$ etc.}
